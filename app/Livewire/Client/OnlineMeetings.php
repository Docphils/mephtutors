...
#[Layout('layouts.app')]
#[Title('Client Online Sessions - MephEd')]
class OnlineMeetings extends Component
{
    ...
    public function render()
    {
        ...
        $query = OnlineMeetingAttendance::with(['meeting.tutor', 'meeting.serviceItem'])
            ->where('user_id', auth()->id())
            ->where('role', 'client')
            ->where('attendance_status', $this->tab)
            ->latest();

        $base = OnlineMeetingAttendance::where('user_id', auth()->id())->where('role', 'client');

        return view('livewire.client.online-meetings', [
