<main>
    <section class="relative min-h-[32rem] bg-cover bg-center"
        style="background-image: url('{{ asset('images/bootcamp-header.jpeg') }}')">
        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 items-start">
                <header class="text-white max-w-xl order-2 lg:order-1">
                    <p class="text-sm font-semibold tracking-wide uppercase text-cyan-200">Beginner-Friendly Tech Skills
                        Bootcamp</p>
                    <h1 class="mt-2 text-3xl sm:text-4xl lg:text-5xl font-black leading-tight">Build practical digital
                        skills for real-world opportunities</h1>
                    <p class="mt-3 text-sm sm:text-base text-slate-100/90">Start your learning journey in software
                        development, product, data, design, and other high-demand tech pathways.</p>
                    <p class="mt-2 text-sm sm:text-base font-semibold text-cyan-200">New cohorts open regularly</p>
                </header>

                <aside class="order-1 lg:order-2">
                    <section
                        class="bg-white text-slate-900 rounded-2xl shadow-2xl p-4 sm:p-5 max-h-[75vh] overflow-y-auto"
                        aria-label="Bootcamp Enrollment Form">
                        <h2 class="text-xl font-bold mb-3">Enroll Now</h2>

                        @if (session()->has('success'))
                            <aside class="bg-green-50 text-green-700 p-2 rounded-lg mb-2 text-sm" id="success-message"
                                role="status">
                                {{ session('success') }}
                            </aside>
                        @endif
                        ...
                        <form wire:submit.prevent="save" class="space-y-2">
                            <div>
                                <label for="service_item_id" class="block text-gray-700 font-semibold">Bootcamp
                                    Track</label>
                            ...
                            <div>
                                ...
                                <select wire:model="cohort_id" id="cohort_id" name="cohort_id"
                                    class="w-full border rounded-lg focus:outline-none focus:ring"
                                    @disabled(empty($availableCohorts)) required>
                                    <option value="">
                                        {{ empty($availableCohorts) ? 'Select a track first' : 'Select a cohort' }}
                                    </option>
            ...
            <ul class="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <li class="max-w-sm w-full mx-auto text-left list-none">
                    <article>
                        ...
                        <p>Explore in-demand pathways including software, data, product, design, and related digital
                            skills.</p>
                    </article>
                </li>
                ...
                <li class="max-w-sm w-full mx-auto text-left list-none">
                    <article>
                        ...
                        <p>Build portfolio-ready work and receive guidance for freelance, internship, and career
                            opportunities.</p>
                    </article>
                </li>
            </ul>
