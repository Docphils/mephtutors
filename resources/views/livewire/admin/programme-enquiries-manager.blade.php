<div class="min-h-screen bg-slate-50 p-3 sm:p-4 lg:p-6">
    <x-slot name="header">
        <div class="flex flex-col gap-2">
            <h2 class="text-2xl font-black tracking-tight text-slate-800">Intervention <span
                    class="text-cyan-600">Enquiries</span></h2>
            <p class="text-sm text-slate-500">Review incoming intervention requests, assign tutors, and update delivery
                status.</p>
        </div>
    </x-slot>
            ...
            <div class="grid gap-3 md:grid-cols-4">
                <div class="md:col-span-2">
                    <input wire:model.live.debounce.300ms="search" type="text"
                        placeholder="Search learner, parent, contact, or intervention"
                ...
                <article
                    ...
                    <div class="mt-4 grid grid-cols-2 gap-3 text-sm">
                        ...
                        <div class="rounded-2xl bg-slate-50 p-3">
                            ...
                            <p class="text-xs text-slate-500">
                                {{ $enquiry->user?->userProfile?->phone ?? $enquiry->parent_phone ?: 'No phone' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 grid grid-cols-2 gap-3 text-xs text-slate-600">
                        <div>
                            ...
                            <p class="mt-1">{{ ucfirst($enquiry->lesson_mode) }} |
                                {{ $enquiry->preferred_frequency ?: 'Flexible' }}</p>
                            <p>{{ $enquiry->preferred_duration ?: 'Flexible duration' }}</p>
                        </div>
                        <div>
                            <p class="font-black uppercase text-slate-400">Location</p>
                            <p class="mt-1">{{ $enquiry->city_area ?: 'City not set' }},
                    ...
                    <div class="mt-4 flex items-center justify-between gap-2 border-t border-slate-100 pt-4">
                        <div class="space-y-1">
                            <span
                                class="inline-flex rounded-full px-2.5 py-1 text-[10px] font-black uppercase {{ $paymentTone }}">
    ...
    @if ($showDetailsModal && $selectedEnquiry)
        @php
            ...
            $clientAdjustmentNote = $selectedEnquiry->meta['client_adjustment_note'] ?? null;
            $clientDeclineNote = $selectedEnquiry->meta['client_completion_decline_note'] ?? null;
            $clientAdjustmentRequestedAt = $selectedEnquiry->meta['client_adjustment_requested_at'] ?? null;
            $clientCompletionDeclinedAt = $selectedEnquiry->meta['client_completion_declined_at'] ?? null;
            ...
            $paymentTone =
                ($selectedEnquiry->payment_status ?? 'pending') === 'paid'
                    ? 'bg-emerald-100 text-emerald-700'
                    : 'bg-slate-100 text-slate-600';
            $meta = (array) ($selectedEnquiry->meta ?? []);
            $clientMetaNotes = collect($meta)->filter(fn($mval, $mkey) => \Illuminate\Support\Str::startsWith($mkey, 'client') && !empty($mval));
        ...
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div
                class="relative flex max-h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
                <div
                    class="flex items-start justify-between gap-4 border-b border-slate-100 bg-slate-900 px-6 py-5 text-white">
                    <div>
                        ...
                        <h3 class="mt-1 text-2xl font-black">{{ $selectedEnquiry->programme?->name ?? 'Intervention' }}
                        </h3>
                        <p class="mt-1 text-xs text-slate-300">Submitted
                            {{ $selectedEnquiry->created_at?->format('d M Y, H:i') }}</p>
                    </div>
                ...
                <div class="grid gap-6 overflow-y-auto p-6 lg:grid-cols-3">
                    <section class="space-y-4 lg:col-span-2">
                        <div class="grid gap-4 md:grid-cols-2">
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                ...
                                <p class="mt-2 text-sm font-bold text-slate-800">{{ $selectedEnquiry->learner_name }}
                                </p>
                                <p class="text-xs text-slate-600">
                                    {{ $selectedEnquiry->class_level ?: 'Class level not provided' }}</p>
                                <p class="mt-2 text-xs text-slate-600">School:
                                    {{ $selectedEnquiry->school_name ?: 'Not provided' }}</p>
                            </div>
                            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                ...
                                <p class="mt-2 text-sm font-bold text-slate-800">
                                    {{ $selectedEnquiry->user?->name ?? $selectedEnquiry->parent_name ?: 'Not provided' }}
                                </p>
                                <p class="text-xs text-slate-600">Email:
                                    {{ $selectedEnquiry->user?->email ?: 'Not available' }}</p>
                                <p class="text-xs text-slate-600">Phone:
                                    {{ $selectedEnquiry->user?->userProfile?->phone ?? $selectedEnquiry->parent_phone ?: 'Not available' }}
                                </p>
                                <p class="mt-2 text-xs text-slate-600">Address:
                                    {{ $selectedEnquiry->parent_address ?: 'Not provided' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="rounded-2xl border border-slate-200 p-4">
                            ...
                            <div class="mt-4 grid gap-4 md:grid-cols-2 text-xs text-slate-700">
                                <div>
                                    ...
                                    <p class="mt-1">{{ $selectedEnquiry->weak_areas ?: 'No weak areas provided.' }}
                                    </p>
                                </div>
                                <div>
                                    <p class="font-black uppercase text-slate-400">Recent Performance Notes</p>
                                    <p class="mt-1">
                                        {{ $selectedEnquiry->recent_performance_notes ?: 'No performance notes provided.' }}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="rounded-2xl border border-slate-200 p-4">
                            ...
                            <p class="mt-3 text-xs text-slate-600">
                                Location: {{ $selectedEnquiry->city_area ?: 'City not provided' }},
                                {{ $selectedEnquiry->state ?: 'State not provided' }}
                            </p>
                        </div>
                                    ...
                                    <div class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-xs">
                                        <div class="flex items-start justify-between gap-3">
                                            <div>
                                                ...
                                                <p class="text-slate-500">Assigned by
                                                    {{ $assignment->assignedBy?->name ?? 'System' }}</p>
                                                <p class="text-slate-500">Planned start:
                                                    {{ $assignment->start_date?->format('d M Y') ?? 'Not set' }}</p>
                                                <p class="text-slate-500">Completed:
                                                    {{ $assignment->completed_at?->format('d M Y, H:i') ?? 'Not completed' }}</p>
                                            </div>
                                            <span
                                                class="rounded-full bg-slate-200 px-2 py-1 text-[10px] font-black uppercase text-slate-700">
                                                {{ str_replace('_', ' ', $assignment->status) }}
                                            </span>
                                        </div>

                                        @if (!empty($assignment->admin_notes))
                                            <div class="mt-2 rounded-lg border border-slate-200 bg-white p-2">
                                                <p class="text-[10px] font-black uppercase text-slate-400">Admin note</p>
                                                <p class="mt-1 whitespace-pre-line break-words text-slate-700">
                                                    {{ $assignment->admin_notes }}
                                                </p>
                                            </div>
                                        @endif

                                        @if (!empty($assignment->tutor_notes))
                                            <div class="mt-2 rounded-lg border border-cyan-200 bg-cyan-50 p-2">
                                                <p class="text-[10px] font-black uppercase text-cyan-700">Tutor remark</p>
                                                <p class="mt-1 whitespace-pre-line break-words text-cyan-900">
                                                    {{ $assignment->tutor_notes }}
                                                </p>
                                            </div>
                                        @endif
                                    </div>
                    ...
                    <aside class="space-y-4">
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            ...
                            <select wire:model="statusInputs.{{ $selectedId }}"
                        ...
                        <div class="rounded-2xl border border-slate-200 p-4">
                            ...
                            <label class="mt-3 block text-[10px] font-black uppercase text-slate-400">Quote
                                (NGN)</label>
                            <input type="number" step="0.01" wire:model.live="priceInputs.{{ $selectedId }}"
                                class="mt-1 w-full rounded-xl border-slate-200 text-sm"
                                placeholder="Enter approved quote" />

                            <label class="mt-3 block text-[10px] font-black uppercase text-slate-400">Planned Start
                                Date</label>
                            <input type="date" wire:model="startDateInputs.{{ $selectedId }}"
                                class="mt-1 w-full rounded-xl border-slate-200 text-sm" />
                            @error('startDateInputs.' . $selectedId)
                                <p class="mt-2 text-xs font-bold text-rose-600">{{ $message }}</p>
                            @enderror

                            <label class="mt-3 block text-[10px] font-black uppercase text-slate-400">Client Update
                                Note (optional)</label>
                            <textarea wire:model.defer="quoteUpdateNotes.{{ $selectedId }}" rows="3"
                                class="mt-1 w-full rounded-xl border-slate-200 text-sm"
                                placeholder="Comment shown in notifications when returning matched offer."></textarea>

                            <button wire:click="saveQuoteAndDate({{ $selectedId }})" wire:loading.attr="disabled"
                                wire:target="saveQuoteAndDate"
                                class="mt-3 w-full rounded-xl bg-slate-900 px-4 py-2 text-xs font-bold text-white transition hover:bg-black disabled:opacity-60">
                                Save Quote and Date
                            </button>
                        </div>
                        ...
                        <div class="rounded-2xl border border-slate-200 p-4">
                            ...
                            <p class="mt-2 text-[11px] text-slate-500">Reassignment cancels the current active
                                assignment and creates a new one.</p>

                            <label class="mt-3 block text-[10px] font-black uppercase text-slate-400">Tutor</label>
                            <select wire:model="tutorInputs.{{ $selectedId }}"
                                class="mt-1 w-full rounded-xl border-slate-200 text-sm">
                                <option value="">Select a tutor</option>
                                @foreach ($tutors as $tutor)
                                    <option value="{{ $tutor->id }}">
                                        {{ $tutor->tutorProfile?->fullName ?? ($tutor->name ?? 'Tutor') }}</option>
                                @endforeach
                            </select>

                            <label class="mt-3 block text-[10px] font-black uppercase text-slate-400">Admin Note
                                (optional)</label>
                            <textarea wire:model.defer="adminNotes.{{ $selectedId }}" rows="3"
                                class="mt-1 w-full rounded-xl border-slate-200 text-sm"
                                placeholder="Add onboarding or assignment notes for tutor"></textarea>

                            <button wire:click="reassignTutor({{ $selectedId }})" wire:loading.attr="disabled"
                                wire:target="reassignTutor"
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs text-slate-600">
                            <p class="font-black uppercase text-slate-400">Payment and Notes Summary</p>
                            <p class="mt-2">Quote:
                                {{ $selectedEnquiry->price_quote ? 'NGN ' . number_format((float) $selectedEnquiry->price_quote, 2) : 'Not set' }}
                            </p>
                            <p class="mt-1">Payment: {{ ucfirst($selectedEnquiry->payment_status ?? 'pending') }}
                            </p>
                            <p class="mt-1">Reference: {{ $selectedEnquiry->payment_reference ?: 'Not available' }}
                            </p>
                            <p class="mt-1">Planned Start:
                                {{ $latestSelectedAssignment?->start_date?->format('d M Y') ?? 'Not set' }}</p>

                            @if (!empty($selectedEnquiry->weak_areas))
                                <p class="mt-2"><span class="font-black text-slate-500">Client Focus Areas:</span>
                                    {{ $selectedEnquiry->weak_areas }}</p>
                            @endif

                            @if ($clientAdjustmentNote)
                                <p class="mt-2"><span class="font-black text-slate-500">Client Adjustment:</span>
                                    {{ $clientAdjustmentNote }}</p>
                                <p class="text-[11px] text-slate-500">
                                    Requested:
                                    {{ $clientAdjustmentRequestedAt && strtotime((string) $clientAdjustmentRequestedAt) ? \Carbon\Carbon::parse($clientAdjustmentRequestedAt)->format('d M Y, H:i') : 'Time not available' }}
                                </p>
                            @endif

                            @if ($clientDeclineNote)
                                <p class="mt-2"><span class="font-black text-slate-500">Client Completion
                                        Decline:</span> {{ $clientDeclineNote }}</p>
                                <p class="text-[11px] text-slate-500">
                                    Submitted:
                                    {{ $clientCompletionDeclinedAt && strtotime((string) $clientCompletionDeclinedAt) ? \Carbon\Carbon::parse($clientCompletionDeclinedAt)->format('d M Y, H:i') : 'Time not available' }}
                                </p>
                            @endif

                            @foreach ($clientMetaNotes as $mkey => $mval)
                                @if ($mkey !== 'client_adjustment_note' && $mkey !== 'client_completion_decline_note')
                                    @php
                                        $displayVal = is_array($mval)
                                            ? json_encode($mval, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE)
                                            : (string) $mval;
                                        $label = ucwords(str_replace(['_', 'client'], ' ', $mkey));
                                    @endphp
                                    <p class="mt-2">
                                        <span class="font-black text-slate-500">{{ $label }}:</span>
                                        {{ $displayVal !== '' ? $displayVal : 'Not provided' }}
                                    </p>
                                @endif
                            @endforeach

                            <div class="mt-3 border-t border-slate-200 pt-3">
                                <p class="font-black uppercase text-slate-400">Tutor Remarks History</p>
                                @forelse ($tutorRemarks as $remarkAssignment)
                                    <div class="mt-2 rounded-lg border border-cyan-200 bg-cyan-50 p-2">
                                        <p class="font-bold text-cyan-900">
                                            {{ $remarkAssignment->tutor?->tutorProfile?->fullName ?? ($remarkAssignment->tutor?->name ?? 'Tutor') }}
                                        </p>
                                        <p class="text-[11px] text-cyan-800">
                                            {{ $remarkAssignment->completed_at?->format('d M Y, H:i') ?? ($remarkAssignment->updated_at?->format('d M Y, H:i') ?? 'Date not available') }}
                                            • {{ str_replace('_', ' ', $remarkAssignment->status) }}
                                        </p>
                                        <p class="mt-1 whitespace-pre-line break-words text-cyan-900">
                                            {{ $remarkAssignment->tutor_notes }}
                                        </p>
                                    </div>
                                @empty
                                    <p class="mt-2 text-xs text-slate-500">No tutor remarks yet.</p>
                                @endforelse
                            </div>
                        </div>
                    </aside>
