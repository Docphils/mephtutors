<div class="p-4 sm:p-6 bg-cyan-100 min-h-screen">
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-black text-slate-800">Tutor <span class="text-cyan-600">Online Sessions</span></h2>
            <p class="text-xs text-slate-500 font-medium">Your scheduled, attended, and missed virtual class records.</p>
        </div>
    </x-slot>
        ...
        @forelse($records as $record)
            <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
                ...
                <div class="mt-4 text-sm text-slate-600 space-y-1">
                    <p><strong>Client:</strong> {{ $record->meeting->client?->name ?? 'N/A' }}</p>
