...
<head>
    ...
    <title>Intervention Update</title>
</head>
...
<body style="margin:0;padding:24px;background:#e2e8f0;font-family:Arial,Helvetica,sans-serif;color:#0f172a;">
    <div
        ...
        <div style="padding:22px 24px;">
            <p style="margin-top:0;">Hello {{ $enquiry->user?->name ?? ($enquiry->parent_name ?? 'Client') }},</p>
            ...
            <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:14px;">
                ...
                <p style="margin:0 0 8px;"><strong>Payment:</strong>
                    {{ strtoupper((string) ($enquiry->payment_status ?? 'pending')) }}</p>
                <p style="margin:0;"><strong>Tutor:</strong>
                    {{ $context['tutor_name'] ?? ($enquiry->activeAssignment?->tutor?->tutorProfile?->fullName ?? ($enquiry->activeAssignment?->tutor?->name ?? 'Pending assignment')) }}
                </p>
            </div>

            @if (!empty($context['review_deadline']))
                <p style="margin:14px 0 0;"><strong>Review Deadline:</strong> {{ $context['review_deadline'] }}</p>
            @endif
